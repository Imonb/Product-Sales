...
public class EditProductServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!currentProduct.getUpdateDatetime().equals(currentUpdateTime)) {
			// エラーメッセージをセッションに設定
			//request.getSession().setAttribute("errorforOCC", "商品を更新できません。");
			request.setAttribute("errorforOCC", "商品を更新できません。");
			ArrayList<Product> productList = productDao.getAllProduct();
			request.setAttribute("product_list", productList);
			request.getRequestDispatcher("/WEB-INF/view/IndexProduct.jsp").forward(request, response);
